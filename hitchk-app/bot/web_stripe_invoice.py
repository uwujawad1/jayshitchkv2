...
from gates.stripe_invoice import stripe_invoice_check, parse_invoice_url

async def main():
    if len(sys.argv) < 3:
        print(json.dumps({"error": "Usage: web_stripe_invoice.py <invoice_url> <cc|mm|yy|cvv>"}))
        return

    invoice_url = sys.argv[1]
    card_str = sys.argv[2]

    ref = parse_invoice_url(invoice_url)
    if not ref:
        print(json.dumps({"error": "Invalid Stripe invoice URL"}))
        return
    ...
    try:
        status, msg, card_info, elapsed, cached = await stripe_invoice_check(
            cc, mm, yy, cvv, invoice_url,
        ...
        if cached:
            safe_cache = {}
            for k in ["pk", "ek", "pi_id", "pi_cs", "invoice_id", "merchant", "amount", "currency",
