    ...
    try:
        ...
        if result1['__typename'] == 'CheckpointDenied':
            ...
            if redirect_url:
                ...
                try:
                    from captcha_solver import solve_checkpoint
                    ...
                    if solved:
                        ...
                        _update_qt(result1)
                        if result1['__typename'] == 'CheckpointDenied':
                            return None, "Captcha required (retry failed)", gateway_display_name
                    else:
                        return None, "Captcha solve failed", gateway_display_name
                except Exception as e:
                    logger.warning(f"Captcha solving error: {e}")
                    return None, "Captcha required", gateway_display_name
            else:
                return None, "Captcha required", gateway_display_name
