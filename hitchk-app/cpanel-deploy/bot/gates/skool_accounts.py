...
async def _ensure_client(email):
...
async def get_authed_client(account, force_refresh=False):
    email = account["email"]
    password = account["password"]

    data = await _ensure_client(email)
    client = data["client"]
...
def _save_user_skool_accounts(data):
    global _user_skool_cache, _user_skool_ts
    try:
        with open(USER_SKOOL_FILE, "w") as f:
