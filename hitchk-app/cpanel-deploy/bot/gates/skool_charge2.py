...
logger = logging.getLogger("skool_charge2")
...
async def _resolve_group_id(client):
    global GROUP_ID
    if GROUP_ID:
        return GROUP_ID

    now = time.time()
    if _group_id_cache["id"] and (now - _group_id_cache["ts"]) < 600:
        GROUP_ID = _group_id_cache["id"]
        return GROUP_ID

    try:
        r = await client.get(f"{SKOOL_API}/groups/{GROUP_NAME}", timeout=REQUEST_TIMEOUT)
        if r.status_code == 200:
            data = r.json()
            gid = data.get("id") or data.get("groupId") or data.get("group_id")
            if gid:
                GROUP_ID = gid
                _group_id_cache["id"] = gid
                _group_id_cache["ts"] = now
                return GROUP_ID
    except Exception:
        pass

    try:
        r = await client.get(f"{SKOOL_API}/groups/{GROUP_NAME}/about", timeout=REQUEST_TIMEOUT)
        if r.status_code == 200:
            data = r.json()
            gid = data.get("id") or data.get("groupId") or data.get("group_id")
...
async def _leave_group(client, gid=None):
    g = gid or GROUP_ID
    if g:
        try:
            await client.post(f"{SKOOL_API}/groups/{g}/cancel-join-group-paid", timeout=15)
        except Exception:
            pass
    try:
        await client.post(f"{SKOOL_API}/groups/{GROUP_NAME}/cancel-join", timeout=15)
    except Exception:
        pass
    try:
        await client.post(f"{SKOOL_API}/groups/{GROUP_NAME}/leave", timeout=15)
    except Exception:
        pass
    logger.debug(f"Auto-leave done for {GROUP_NAME}")


async def skool_charge2_check(cc, mm, yy, cvv, proxy=None, user_id=None, is_admin=False):
    tried_emails = set()
    last_result = None
    ...
    for _ in range(3):
        ...
        async with lock:
            result = await _check_with_account(cc, mm, yy, cvv, account)
        ...
        if "Skool Account Login Error" in result:
            ...
            if fallback and fallback["email"] not in tried_emails:
                ...
                async with lock2:
                    result = await _check_with_account(cc, mm, yy, cvv, fallback)
...
async def _check_with_account(cc, mm, yy, cvv, account):
    start = time.time()
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            client = await get_authed_client(account, force_refresh=force_refresh)
            if not client:
                ...
                return f"Error - Your Skool Account Login Error [{elapsed}s]"

            gid = await _resolve_group_id(client)
            if not gid:
                if attempt < MAX_RETRIES - 1:
                    await asyncio.sleep(random.uniform(RETRY_DELAY_MIN, RETRY_DELAY_MAX))
                    continue
                elapsed = round(time.time() - start, 2)
                return f"Error - Could not resolve group ID [{elapsed}s]"

            await _leave_group(client, gid)
            ...
            if r_join.status_code == 200:
                ...
                if cs:
                    ...
                    if pi_status == "succeeded":
                        return f"Approved - Charged $1 | {info} [{elapsed}s]"
            ...
            if client and joined_group:
                await _leave_group(client, gid if 'gid' in dir() else None)
